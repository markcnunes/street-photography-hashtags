...
import { TKeywords } from 'api/data';
import Button from 'components/Button';
import KeywordsList from 'components/KeywordsList';
import { rgba } from 'polished';
import { useState } from 'react';

interface IVerifyKeyword {
  /**
   * Array of keywords.
   */
  keywords: TKeywords[];
 ...
 */
const VerifyKeyword: React.FC<IVerifyKeyword> = ({ keywords }) => {
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [keywordsAvalilable, setKeywordsAvalilable] = useState<TKeywords[]>([]);
  ...
  const handleAddOption = (e: React.FormEvent) => {
    ...
    const arrayKeywords: TKeywords[] = keyword
      .split(/[ ]+/)
    ...
    const hasKeyword: TKeywords[] = keywords.filter((item) => {
    ...
    const newKeywords: TKeywords[] = arrayKeywords
      .filter((x: TKeywords) => !hasKeyword.includes(x))
